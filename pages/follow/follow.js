...
Page({
  ...
  getAjax() {
    ...
    const _this = this;
		...
		util.get(`${selectFollowList}`,para).then(res=>{
			if (res.code == 0) {
			  _this.setData({
			    list: res.info.list,
			  });
        ...
        uploadErrorInfoFn(`${selectFollowListTitle}`, `event:进入页面请求;requestParameters:${JSON.stringify(para)};errorinfo:${JSON.stringify(res)}`);
			  let title = "系统通知";
			  let notice = '出错啦';
			  wx.navigateTo({
			    url: '../../other_pages/error/error?title=' + title + "&notice=" + notice,
      ...
      uploadErrorInfoFn(`${selectFollowListTitle}`, `event:进入页面请求;requestParameters:${JSON.stringify(para)};errorinfo:${JSON.stringify(error)}`);
			let title = "系统通知";
			let notice = '出错啦';
			wx.stopPullDownRefresh();
			wx.hideLoading();
